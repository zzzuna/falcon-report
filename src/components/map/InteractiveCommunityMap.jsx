...
export function InteractiveCommunityMap({ report }) {
    ...
    return (
        <section id="community-map" className="mb-10 print:hidden relative">
            ...
            <div className="bg-white/5 backdrop-blur-xl p-4 rounded-2xl border border-white/10 shadow-sm mb-4 flex flex-col sm:flex-row gap-4 justify-between items-center relative z-20">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {['All', 'Upgrade Projects', 'Villas HO'].map(f => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5 ${activeFilter === f ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/5 text-slate-400 hover:bg-white/10 border border-transparent'}`}
                            >
                ...
                <div className="relative max-w-xs w-full">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-black/20 border border-white/10 text-slate-200 rounded-lg text-sm font-medium focus:border-white/10 focus:ring-1 focus:ring-slate-800 outline-none transition"
                    />
                </div>
            </div>

            <div className="relative bg-slate-900 rounded-2xl overflow-hidden border border-white/10 shadow-sm select-none">
                <div className="relative w-full group" style={{ cursor: 'grab' }}>
                    <img
                        src={baseImageUrl}
                        alt="Master Plan Overlay"
                        className="w-full h-auto block opacity-90 pointer-events-none invert hue-rotate-180 brightness-150 contrast-125 saturate-50"
                    />
                        ...
                        >
                            <div className="relative">
                                ...
                                {selectedNode?.id === point.id && (
                                    <div className="absolute -inset-1 rounded-full border-2 border-white/10 animate-ping opacity-50" />
                                )}
                            </div>
                        </div>
                    ))}
                ...
                {/* Map Legend */}
                <div className="md:absolute md:top-4 md:right-4 bg-[#0F172A]/90 md:backdrop-blur border-t md:border border-white/10 p-2 md:p-3 md:rounded-lg md:shadow-md z-20 pointer-events-auto w-full md:w-auto relative flex flex-col gap-1.5 md:gap-2">
                    
                    <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                        <div className="uppercase tracking-wider text-slate-400 text-[10px] font-black md:w-20 shrink-0">Villas</div>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-[#8bc34a] border border-[#689f38] rounded-sm"></div>Handed Over</div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-[#0288d1] border border-[#01579b] rounded-sm"></div>Not Accepted</div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-purple-500 border border-purple-700 rounded-sm"></div>Occupied</div>
                        </div>
                    </div>

                    <div className="hidden md:block border-t border-white/10 my-0"></div>

                    <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                        <div className="uppercase tracking-wider text-slate-400 text-[10px] font-black md:w-20 shrink-0">Projects</div>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-emerald-500 rounded-full border border-white shadow-sm"></div>Completed</div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-orange-500 rounded-full border border-white shadow-sm"></div>In Progress</div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-blue-500 rounded-full border border-white shadow-sm"></div>Planned</div>
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-300 leading-none"><div className="w-3 h-3 bg-rose-500 rounded-full border border-white shadow-sm"></div>Cancelled</div>
                        </div>
                    </div>
                ...
                {hoveredNode && (
                    <div className="absolute pointer-events-none z-30 bg-[#0F172A]/90 backdrop-blur border border-white/10 p-3 rounded-lg shadow-xl"
                        style={{
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                            ...
                            />
                            <span className="text-xs font-semibold text-slate-400 uppercase">{hoveredNode.status || 'N/A'}</span>
                        </div>
                        {hoveredNode.comments && (
                            <p className="text-xs font-medium text-slate-600 mt-2 max-w-[200px] border-t border-white/10 pt-2 leading-relaxed">
                                {hoveredNode.comments}
                            </p>
                        )}
                    </div>
            ...
            {selectedNode && (
                <div className="fixed inset-y-0 right-0 w-full md:w-96 bg-white shadow-2xl z-50 border-l border-white/10 transform transition-transform duration-300 flex flex-col pt-16 md:pt-0">
                    <div className="p-6 border-b border-white/10 flex justify-between items-start bg-slate-50">
                        <div>
                        ...
                        <button onClick={() => setSelectedNode(null)} className="p-2 hover:bg-slate-200 rounded-full transition bg-white border border-white/10">
                    ...
                    <div className="p-6 flex-1 overflow-y-auto space-y-6">
                        <div>
                            ...
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 border border-white/10 rounded text-sm font-bold text-slate-300 uppercase">
                                <span
                        ...
                        {(selectedNode.notes || selectedNode.description) && (
                            <div>
                                ...
                                <p className="text-sm font-medium text-slate-600 leading-relaxed p-4 bg-slate-50 rounded-lg border border-white/10">
                                    {selectedNode.notes || selectedNode.description}
                                </p>
                            </div>
                        )}
